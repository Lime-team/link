...
@connection
async def set_user(session, tg_id: int, description: str = 'нет', medals: str = 'нет') -> Optional[ChatUser]:
    try:
        user = await session.scalar(select(ChatUser).filter_by(tg_id=tg_id))

        if not user:
            new_user = ChatUser(tg_id=tg_id, description=description, medals=medals)
...
@connection
async def remove_user(session, tg_id: str) -> Optional[ChatUser]:
    try:
        user = await session.get(ChatUser, tg_id)
        if not user:
